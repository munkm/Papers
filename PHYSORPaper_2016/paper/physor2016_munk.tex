...
%------------------------------------------------------------------------------
\title{CADIS-$\Omega$: AN ANGLE-INFORMED HYBRID METHOD FOR DEEP-PENETRATION RADIATION TRANSPORT}
%
% ...and authors
%
\author{ 
...
% should we reference the two papers that one of our reviewers suggested? Even though they don't have to do with deep-penetration RT? 
% Is FOM equation wrong? 
% Should we consider only putting in labyrinth II results? 
% I'm worried about presenting the timing of all of the problems I ran. For labyrinth I, they were 3 minutes, 46 minutes, and 6 minutes. For Labyrinth II they were 5 minutes, 19 minutes, and 30 minutes. 
...
The contributon flux includes both forward and adjoint information, expressing the importance of a particle to the solution.
This contains more information than the adjoint flux, which only indicates the importance of a particle to a response function.
...
We are interested in contributons because they include angular information from both the forward and adjoint calculation. % Or some statement that is like this...I think clarifying above will help me understand how to state this. We don't want to talk about contributons as generally but to connect it to VR and this method.

% Come up with good concluding statements here that will help transition to past work section. 
...
Both CADIS and FW-CADIS use an adjoint scalar flux to generate biasing parameters for variance reduction as show in Eqns.~\eqref{}. 
...
An importance map %from the adjoint flux will merely have greater importance with increasing proximity to the adjoint source, while the imporance map generated from 
...
The deterministic methods used the 27g19n shielding cross section library, a step characteristic (sc) spatial solver, quadruple range (qr) quadrature, a $P_N$ order of 2, (SN, PN, 135000 spatial cells, SPATIAL SOLVER). In ADVANTG, the quadruple range quadrature is set with four azimuthal and four polar angles per octant, and a default quadrature order of 10. 
...
Separating the detector response into the flux and response function ($\Sigma_{T}$), as in Figure~\ref{fig::tallyproducts}, we can gain some insight as to why the low energy region is so problematic for these methods. Prior to entering the lowest energy group   
...
Both figures~\ref{fig::adjoint_fluxes_group26} and ~\ref{fig::adjoint_fluxes_group11} exhibit some ray effects behind the shield. Comparing the flux map between CADIS and  CADIS-$\Omega$,  CADIS-$\Omega$ does not seem to suffer any more from ray effects than traditional CADIS. Depending on the extent to which CADIS-$\Omega$ can reduce ray effects will be 
...
Based on both theory and the method's performance in this demonstration problem, we expect that this method will perform well in problems with strong streaming paths and less well in problems with significant scattering effects. 
