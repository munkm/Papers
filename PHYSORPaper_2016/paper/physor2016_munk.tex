...
Many modern radiation transport codes offer variance reduction capabilities that employ an importance map\textemdash a measure of how important a particular region in a Monte Carlo simulation is to the tally being computed\textemdash to perform variance reduction. Importance maps can be generated in a number of methods, but a particularly effective method to generate an importance map is related to the adjoint form of the transport equation. The steady state, fixed-source forms of the forward and adjoint transport equation are
